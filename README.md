...
\`\`\`
                          ...
                          │                                          │
  Internet ──────────────►│  ┌────────────────────────────────────┐ │
                          │  │  gluetun (PIA VPN + port forward)   │ │
                          │  │   └── Deluge (torrent client)       │ │
                          │  │        no traffic leaves without VPN │ │
                          │  └────────────────────────────────────┘ │
                          │                                          │
                          │  Plex ── Bazarr ── FileBrowser           │
                          ...
                          │  Storage:                                │
                          │   /  (250 GB)  → OS + configs            │
                          │   /mnt/data_500gb → downloads (temp)     │
                          │   /mnt/data_1tb   → media library        │
                          └─────────────────────────────────────────┘
                                          │
                          ┌───────────────┴───────────────┐
                          ...
                          └───────────────┬───────────────┘
                                          │
                    Mac / iPhone / Smart TVs (anywhere, any network)
\`\`\`
...
- **500 GB** → active downloads (temporary)
- **1 TB** → media library (Plex) + config backups
...
\`\`\`
Add torrent + label (movies / tv / misc)
        ↓
Downloads via VPN to 500 GB disk
        ↓
On completion → moved to correct folder on 1 TB disk
        ↓
Plex auto-scans → appears in library    (movies/tv)
FileBrowser → download to any device     (misc: books, etc.)
\`\`\`
...
│   ├── downloads/             # gluetun + Deluge (VPN-isolated)
│   ├── plex/
│   ├── bazarr/
│   ├── filebrowser/
│   ├── adguard/
...
1. Copy each \`.env.example\` → \`.env\` and fill in your own values (VPN credentials, tokens, etc.)
...
- **Config-as-backup** — data is disposable, but configuration is the real asset; hence daily backups + this off-site repo.
